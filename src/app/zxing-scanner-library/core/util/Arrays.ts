...
export default class Arrays {
   ...
   */
    public static fill(a: Int32Array | Uint8Array | any[], val: number): void {
   ...
   */
    public static fillWithin(a: Int32Array, fromIndex: number, toIndex: number, val: number): void {
  ...
  public static asList<T = any>(...args: T[]): T[] {
    ...
    let arr = Array.from({ length: rows });
        ...
        return arr.map(() => Array.from<T>({ length: cols }).fill(value!));
  }
    ...
    return arr.map(() => Int32Array.from({ length: cols }).fill(value!));
  }
