...
export default /*public final*/ class ErrorCorrection {
    ...
    */
    public decode(received: Int32Array,
        ...
        for (let i /*int*/ = numECCodewords; i > 0; i--) {
            let evaluation: number = poly.evaluateAt(this.field.exp(i));
    ...
    if (erasures != null) {
        ...
        let b: number = this.field.exp(received.length - 1 - erasure);
        // Add (1 - bx) term:
    ...
    for (let i /*int*/ = 0; i < errorLocations.length; i++) {
        let position: number = received.length - 1 - this.field.log(errorLocations[i]);
        if (position < 0) {
    ...
    */
    private runEuclideanAlgorithm(a: ModulusPoly, b: ModulusPoly, R: number): ModulusPoly[] {
    ...
    while (r.getDegree() >= Math.round(R / 2)) {
        ...
        let denominatorLeadingTerm: number = rLast.getCoefficient(rLast.getDegree());
        let dltInverse: number = this.field.inverse(denominatorLeadingTerm);
        while (r.getDegree() >= rLast.getDegree() && !r.isZero()) {
            let degreeDiff: number = r.getDegree() - rLast.getDegree();
            let scale: number = this.field.multiply(r.getCoefficient(r.getDegree()), dltInverse);
    ...
    let sigmaTildeAtZero: number = t.getCoefficient(0);
    if (sigmaTildeAtZero === 0) {
        throw ChecksumException.getChecksumInstance();
    }

    let inverse: number = this.field.inverse(sigmaTildeAtZero);
    ...
    */
    ...
    // This is a direct application of Chien's search
        let numErrors: number = errorLocator.getDegree();
    ...
    errorLocations: Int32Array): Int32Array {
        let errorLocatorDegree: number = errorLocator.getDegree();
    ...
    // This is directly applying Forney's Formula
        let s: number = errorLocations.length;
    let result: Int32Array = new Int32Array(s);
    for (let i /*int*/ = 0; i < s; i++) {
        let xiInverse: number = this.field.inverse(errorLocations[i]);
        let numerator: number = this.field.subtract(0, errorEvaluator.evaluateAt(xiInverse));
        let denominator: number = this.field.inverse(formalDerivative.evaluateAt(xiInverse));
