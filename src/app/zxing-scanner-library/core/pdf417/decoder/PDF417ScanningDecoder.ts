...
export default /*public final*/ class PDF417ScanningDecoder {
  ...
  /*final*/ static CODEWORD_SKEW_SIZE: number = 2;

  /*final*/ static MAX_ERRORS: number = 3;
  /*final*/ static MAX_EC_CODEWORDS: number = 512;
  ...
  public static decode(image: BitMatrix,
    ...
    imageBottomRight: ResultPoint,
      minCodewordWidth: number,
      maxCodewordWidth: number): DecoderResult {
    ...
    detectionResult.setBoundingBox(boundingBox);
      let maxBarcodeColumn: number = detectionResult.getBarcodeColumnCount() + 1;
    ...
    for (let barcodeColumnCount /*int*/ = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
        let barcodeColumn: number = leftToRight ? barcodeColumnCount : maxBarcodeColumn - barcodeColumnCount;
      ...
      detectionResult.setDetectionResultColumn(barcodeColumn, detectionResultColumn);
        ...
        let previousStartColumn: number = startColumn;
  ...
  private static adjustBoundingBox(rowIndicatorColumn: DetectionResultRowIndicatorColumn): BoundingBox {
    ...
    if (rowHeights == null) {
      ...
      let maxRowHeight: number = PDF417ScanningDecoder.getMax(rowHeights);
      let missingStartRows: number = 0;
    ...
    private static getMax(values: Int32Array): number {
  ...
  private static getRowIndicatorColumn(image: BitMatrix,
    ...
    leftToRight: boolean,
      minCodewordWidth: number,
      maxCodewordWidth: number): DetectionResultRowIndicatorColumn {
    ...
    for (let i /*int*/ = 0; i < 2; i++) {
        let increment: number = i === 0 ? 1 : -1;
        let startColumn: number = Math.trunc(<number>Math.trunc(startPoint.getX()));
        for (let imageRow /*int*/ = Math.trunc(<number>Math.trunc(startPoint.getY())); imageRow <= boundingBox.getMaxY() &&
  ...
  private static adjustCodewordCount(detectionResult: DetectionResult, barcodeMatrix: BarcodeValue[][]): void {
    ...
    let numberOfCodewords: Int32Array = barcodeMatrix01.getValue();
      let calculatedNumberOfCodewords: number = detectionResult.getBarcodeColumnCount() *
  ...
  private static createDecoderResult(detectionResult: DetectionResult): DecoderResult {
    ...
    let codewords: Int32Array = new Int32Array(detectionResult.getBarcodeRowCount() * detectionResult.getBarcodeColumnCount());
      let ambiguousIndexValuesList: /*List<int[]>*/ Array<Int32Array> = [];
      let ambiguousIndexesList: /*Collection<Integer>*/ Array<number> = new Array<number>();
    for (let row /*int*/ = 0; row < detectionResult.getBarcodeRowCount(); row++) {
      for (let column /*int*/ = 0; column < detectionResult.getBarcodeColumnCount(); column++) {
        let values: Int32Array = barcodeMatrix[row][column + 1].getValue();
          let codewordIndex: number = row * detectionResult.getBarcodeColumnCount() + column;
        if (values.length === 0) {
   ...
   * @throws ChecksumException
   */
    private static createDecoderResultFromAmbiguousValues(ecLevel: number,
  ...
  private static createBarcodeMatrix(detectionResult: DetectionResult): BarcodeValue[][] {
    ...
    for (let detectionResultColumn /*DetectionResultColumn*/ of detectionResult.getDetectionResultColumns()) {
      if (detectionResultColumn != null) {
        for (let codeword /*Codeword*/ of detectionResultColumn.getCodewords()) {
          if (codeword != null) {
              let rowNumber: number = codeword.getRowNumber();
            if (rowNumber >= 0) {
    ...
    private static isValidBarcodeColumn(detectionResult: DetectionResult, barcodeColumn: number): boolean {
  ...
  private static getStartColumn(detectionResult: DetectionResult,
      ...
      imageRow: number,
      leftToRight: boolean): number {
      let offset: number = leftToRight ? 1 : -1;
  ...
  private static detectCodeword(image: BitMatrix,
    ...
    leftToRight: boolean,
      ...
      imageRow: number,
      minCodewordWidth: number,
      maxCodewordWidth: number): Codeword {
    ...
    if (moduleBitCount == null) {
      ...
      let codewordBitCount: number = MathUtils.sum(moduleBitCount);
    if (leftToRight) {
      endColumn = startColumn + codewordBitCount;
    } else {
      for (let i /*int*/ = 0; i < moduleBitCount.length / 2; i++) {
          let tmpCount: number = moduleBitCount[i];
    ...
    if (!PDF417ScanningDecoder.checkCodewordSkew(codewordBitCount, minCodewordWidth, maxCodewordWidth)) {
      ...
      let decodedValue: number = PDF417CodewordDecoder.getDecodedValue(moduleBitCount);
      let codeword: number = PDF417Common.getCodeword(decodedValue);
    if (codeword === -1) {
      return null;
    }
  ...
  private static getModuleBitCount(image: BitMatrix,
    ...
    leftToRight: boolean,
      startColumn: number,
      imageRow: number): Int32Array {
      let imageColumn: number = startColumn;
    let moduleBitCount: Int32Array = new Int32Array(8);
      let moduleNumber: number = 0;
      let increment: number = leftToRight ? 1 : -1;
    ...
    private static getNumberOfECCodeWords(barcodeECLevel: number): number {
  ...
  private static adjustCodewordStartColumn(image: BitMatrix,
      ...
      maxColumn: number,
    leftToRight: boolean,
      codewordStartColumn: number,
      imageRow: number): number {
      let correctedStartColumn: number = codewordStartColumn;
      let increment: number = leftToRight ? -1 : 1;
    ...
    private static checkCodewordSkew(codewordSize: number, minCodewordWidth: number, maxCodewordWidth: number): boolean {
   ...
   * @throws ChecksumException
   */
    private static decodeCodewords(codewords: Int32Array, ecLevel: number, erasures: Int32Array): DecoderResult {
    if (codewords.length === 0) {
      throw FormatException.getFormatInstance();
    }

      let numECCodewords: number = 1 << (ecLevel + 1);
      let correctedErrorsCount: number = PDF417ScanningDecoder.correctErrors(codewords, erasures, numECCodewords);
   ...
   */
    private static correctErrors(codewords: Int32Array, erasures: Int32Array, numECCodewords: number): number {
    if (erasures != null &&
   ...
   * @throws FormatException
   */
    private static verifyCodewordCount(codewords: Int32Array, numECCodewords: number): void {
    ...
    // codewords, but excluding the number of error correction codewords.
        let numberOfCodewords: number = codewords[0];
    ...
    private static getBitCountForCodeword(codeword: number): Int32Array {
    ...
    private static getCodewordBucketNumber(codeword: number | Int32Array): number {
    ...
    private static getCodewordBucketNumber_number(codeword: number): number {
    return PDF417ScanningDecoder.getCodewordBucketNumber(PDF417ScanningDecoder.getBitCountForCodeword(codeword));
  }

    private static getCodewordBucketNumber_Int32Array(moduleBitCount: Int32Array): number {
