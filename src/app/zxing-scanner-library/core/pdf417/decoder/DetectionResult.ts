...
export default /*final*/ class DetectionResult {
  ...
  /*final*/ ADJUST_ROW_NUMBER_SKIP: number = 2;

  private /*final*/  barcodeMetadata: BarcodeMetadata;
  private /*final*/ detectionResultColumns: DetectionResultColumn[];
  private boundingBox: BoundingBox;
    private /*final*/  barcodeColumnCount: number;
  ...
  getDetectionResultColumns(): DetectionResultColumn[] {
    ...
    this.adjustIndicatorColumnRowNumbers(this.detectionResultColumns[this.barcodeColumnCount + 1]);
      let unadjustedCodewordCount: number = PDF417Common.MAX_CODEWORDS_IN_BARCODE;
      let previousUnadjustedCount: number;
    do {
   ...
   */
    private adjustRowNumbersAndGetCount(): number {
        let unadjustedCount: number = this.adjustRowNumbersByRow();
    ...
    private adjustRowNumbersByRow(): number {
    ...
    // number starts and ends.
        let unadjustedCount: number = this.adjustRowNumbersFromLRI();
  ...
  private adjustRowNumbersFromBothRI(): void {
    ...
    private adjustRowNumbersFromRRI(): number {
    if (this.detectionResultColumns[this.barcodeColumnCount + 1] == null) {
      return 0;
    }
        let unadjustedCount: number = 0;
    let codewords: Codeword[] = this.detectionResultColumns[this.barcodeColumnCount + 1].getCodewords();
    for (let codewordsRow /*int*/ = 0; codewordsRow < codewords.length; codewordsRow++) {
      if (codewords[codewordsRow] == null) {
        continue;
      }
        let rowIndicatorRowNumber: number = codewords[codewordsRow].getRowNumber();
        let invalidRowCounts: number = 0;
    ...
    private adjustRowNumbersFromLRI(): number {
    ...
    private static adjustRowNumberIfValid(rowIndicatorRowNumber: number, invalidRowCounts: number, codeword: Codeword): number {
    if (codeword == null) {
    ...
    private adjustRowNumbers(barcodeColumn: number, codewordsRow: number, codewords: Codeword[]): void {
  ...
  private static adjustRowNumber(codeword: Codeword, otherCodeword: Codeword): boolean {
    ...
    getBarcodeColumnCount(): number {
    return this.barcodeColumnCount;
  }

    getBarcodeRowCount(): number {
    return this.barcodeMetadata.getRowCount();
  }

    getBarcodeECLevel(): number {
  ...
  getBoundingBox(): BoundingBox {
    ...
    setDetectionResultColumn(barcodeColumn: number, detectionResultColumn: DetectionResultColumn): void {
    this.detectionResultColumns[barcodeColumn] = detectionResultColumn;
  }

    getDetectionResultColumn(barcodeColumn: number): DetectionResultColumn {
