...
export default /*final*/ class DetectionResultRowIndicatorColumn extends DetectionResultColumn {
  ...
  adjustCompleteIndicatorColumnRowNumbers(barcodeMetadata: BarcodeMetadata): void {
    ...
    let firstRow: number = this.imageRowToCodewordIndex(<number> Math.trunc(top.getY()));
    let lastRow: number = this.imageRowToCodewordIndex(<number> Math.trunc(bottom.getY()));
    // We need to be careful using the average row height. Barcode could be skewed so that we have smaller and
    // taller rows
    // float averageRowHeight = (lastRow - firstRow) / /*(float)*/ barcodeMetadata.getRowCount();
      let barcodeRow: number = -1;
      let maxRowHeight: number = 1;
    ...
    for (let codewordsRow /*int*/ = firstRow; codewordsRow < lastRow; codewordsRow++) {
      ...
      //      }
        ...
        let rowDifference: number = codeword.getRowNumber() - barcodeRow;
  ...
  getBarcodeMetadata(): BarcodeMetadata {
    ...
    for (let codeword /*Codeword*/ of codewords) {
      ...
      codeword.setRowNumberAsRowIndicatorColumn();
        let rowIndicatorValue: number = codeword.getValue() % 30;
        let codewordRowNumber: number = codeword.getRowNumber();
      if (!this._isLeft) {
        codewordRowNumber += 2;
      }
