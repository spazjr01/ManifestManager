...
export default /*public*/ /*final*/ class Detector {
  ...
  private static /*final*/ INDEXES_START_PATTERN = Int32Array.from([0, 4, 1, 5]);
    ...
    private static /*final*/ MAX_AVG_VARIANCE: number = /*0.42f*/ 0.42;
    private static /*final*/ MAX_INDIVIDUAL_VARIANCE: number = /*0.8f*/ 0.8;
  ...
  private static detect( multiple: boolean,  bitMatrix: BitMatrix): Array<ResultPoint[]> {
    ...
    while (row < bitMatrix.getHeight()) {
      ...
      if (vertices[0] == null && vertices[3] == null) {
        ...
        for (const barcodeCoordinate of barcodeCoordinates) {
          if (barcodeCoordinate[1] != null) {
              row = <number> Math.trunc(Math.max(row, barcodeCoordinate[1].getY()));
          }
          if (barcodeCoordinate[3] != null) {
              row = Math.max(row, <number> Math.trunc(barcodeCoordinate[3].getY()));
          }
        }
      ...
      if (vertices[2] != null) {
          column = <number> Math.trunc(vertices[2].getX());
          row = <number> Math.trunc(vertices[2].getY());
      } else {
          column = <number> Math.trunc(vertices[4].getX());
          row = <number> Math.trunc(vertices[4].getY());
      }
    }
  ...
  private static findVertices( matrix: BitMatrix,  startRow: /*int*/ number,  startColumn: /*int*/ number): ResultPoint[]  {
    ...
    if (result[4] != null) {
        startColumn = <number> Math.trunc(result[4].getX());
        startRow = <number> Math.trunc(result[4].getY());
    }
  ...
  private static  findRowsWithPattern( matrix: BitMatrix,
    ...
    if (found) {
      let skippedRowCount = 0;
        let previousRowLoc = Int32Array.from([<number>Math.trunc(result[0].getX()), <number> Math.trunc(result[1].getX())]);
   ...
   */
    private static patternMatchVariance(counters: Int32Array, pattern: Int32Array, maxIndividualVariance: number): number {
    ...
    // more "significant digits".
        let unitBarWidth = <number> total / patternLength;
